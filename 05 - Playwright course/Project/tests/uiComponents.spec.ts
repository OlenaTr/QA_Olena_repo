...
test.describe('From Layouts page', () => {
    ...
    test('radio buttons', async ({page}) => {
       ...
       const radioStatus = await theGridForm.getByRole('radio', {name: "Option 1"}).isChecked()
       ...
       expect(await theGridForm.getByRole('radio', {name: "Option 1"}).isChecked()).toBeTruthy()
    })
...
test('Dialog box', async({page}) => {
    await page.getByText('Tables and Data').click()
